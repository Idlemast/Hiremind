import { getJobBySalt, getCandidateBySalt, getApplications, getThresholds, getJobs } from "@/lib/db";
...
export default async function CandidateJobProfilePage({
  params,
}: {
  ...
  const app = applications.find((a) => a.job.id === job.id) ?? null;
  if (!app) notFound();
  ...
  const templates = getCommTemplates(
    { firstName: candidate.name.split(" ")[0], jobTitle: app.job.title, gaps, matchedSkills: skills },
    decision
  );
    ...
    <div className="p-4 lg:p-xl max-w-7xl mx-auto space-y-lg animate-fade-in">
            ...
            <div className="flex gap-md mt-sm flex-wrap">
              ...
              {candidateSalary && (
                <div className="flex items-center gap-xs text-slate-500">
                  ...
                  {jobBudget && <span className="text-label-caps text-slate-400">/ budget : {jobBudget}</span>}
                </div>
              )}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {applications.map((a) => (
              <div key={a.id} className="flex items-center gap-1">
                <Link
                  href={candidateUrl(candidate.salt!, candidate.name, a.job.salt!, a.job.title)}
                  className={[
      ...
      <div className="grid grid-cols-12 gap-lg">
        ...
        <div className="col-span-12 lg:col-span-4 bg-white p-lg rounded-xl border border-outline-variant shadow-sm flex flex-col justify-between gap-md">
          <div>
            <h3 className="font-label-caps text-label-caps text-secondary uppercase mb-md">Hiring Stage</h3>
          ...
          <div>
            <h3 className="font-label-caps text-label-caps text-secondary uppercase mb-2">Poste</h3>
            <Link href={jobUrl(app.job.salt!, app.job.title)} className="group">
        ...
        <aside className="col-span-12 lg:col-span-4 space-y-lg">
          ...
          <div className="bg-white p-lg rounded-xl border border-outline-variant shadow-sm">
            ...
            <CandidateNotes applicationId={app.id} initialNotes={app.notes ?? ""} />
          </div>

          <div className="bg-white p-lg rounded-xl border border-outline-variant shadow-sm space-y-md">
            <h3 className="font-label-caps text-label-caps text-secondary uppercase">Internal Tags</h3>
            <TagEditor candidateId={candidate.id} initialTags={tags} />
            <div className="pt-md border-t border-slate-100">
              <p className="text-label-caps text-secondary mb-2 uppercase">Source</p>
